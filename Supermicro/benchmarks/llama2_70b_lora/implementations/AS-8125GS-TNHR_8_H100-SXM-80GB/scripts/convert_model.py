...
def get_args():
    ...
    parser.add_argument(
        "--hparams_file",
        type=str,
        default="scripts/megatron_llama_config.yaml",
        required=False,
...
def convert(args):
    ...
    for l in range(int(num_layers)):
        ...
        qkv_proj = model.state_dict()[f"model.layers.{l}.self_attn.qkv_proj.weight"]
        chunk_sizes = (
            head_num * head_size,
            num_query_groups * head_size,
            num_query_groups * head_size,
        )
        q, k, v = qkv_proj.split(chunk_sizes, dim=0)
