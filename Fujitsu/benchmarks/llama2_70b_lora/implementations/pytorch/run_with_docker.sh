...
_cont_mounts=(
    ...
    "--volume=$(realpath run_and_time_mod.sh):/workspace/ft-llm/run_and_time_mod.sh"
)
...
for _experiment_index in $(seq 1 "${NEXP}"); do
  (
    ...
    if [[ $CLEAR_CACHES == 1 ]]; then
      bash -c "echo -n 'Clearing cache on ' && hostname && sync && sudo -S /sbin/sysctl vm.drop_caches=3 < password.txt"
    fi

    docker exec -it ${_config_env[@]} ${_cont_name} ./run_and_time_mod.sh
