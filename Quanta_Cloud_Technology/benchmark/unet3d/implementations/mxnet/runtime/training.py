...
def train(flags, model, train_loader, val_loader, score_fn, sw_inference,
          ...
          transfer_ranks, ctx, callbacks, mllogger, run_start_time, samples_per_epoch):
    rank = comm.Get_rank()
    stop_training = False
    converged = False
    diverged = False
    ...
    for cycle in range(1, max_cycles):
        mllogger.start(key=mllogger.constants.BLOCK_START, sync=False,
                       metadata={mllogger.constants.FIRST_EPOCH_NUM: global_epoch * samples_per_epoch,
                                 mllogger.constants.EPOCH_COUNT: flags.evaluate_every * samples_per_epoch})
        for callback in callbacks:
        ...
        if rank in eval_ranks and (global_epoch >= flags.start_eval_at):
            sbridge.start_eval_prof()
            mllogger.start(key=mllogger.constants.EVAL_START, value=global_epoch * samples_per_epoch, sync=False,
                           unique_log_rank=eval_ranks[0],
                           metadata={mllogger.constants.EPOCH_NUM: global_epoch * samples_per_epoch})
            eval_metrics = evaluate(flags, model, val_loader, sw_inference, score_fn, ctx, eval_comm, global_epoch)
            mllogger.event(key=mllogger.constants.EVAL_ACCURACY, sync=False,
                           value=eval_metrics["mean_dice"], unique_log_rank=eval_ranks[0],
                           metadata={mllogger.constants.EPOCH_NUM: global_epoch * samples_per_epoch})
            mllogger.end(key=mllogger.constants.EVAL_STOP, value=global_epoch * samples_per_epoch,
                         unique_log_rank=eval_ranks[0], sync=False,
                         metadata={mllogger.constants.EPOCH_NUM: global_epoch * samples_per_epoch})
            sbridge.stop_eval_prof()
            ...
            if eval_metrics["mean_dice"] >= flags.quality_threshold:
                if not converged:
                    converged = True
                    mllogger.log_run_stop(status=mllogger.constants.SUCCESS, epoch=global_epoch * samples_per_epoch,
                                          sync=False, unique_log_rank=eval_ranks[0])
            elif eval_metrics["mean_dice"] < 1e-4 and not converged:
                stop_training = True
                diverged = True
                mllogger.log_run_stop(status=mllogger.constants.ABORTED, epoch=global_epoch * samples_per_epoch,
                                      sync=False, unique_log_rank=eval_ranks[0])
