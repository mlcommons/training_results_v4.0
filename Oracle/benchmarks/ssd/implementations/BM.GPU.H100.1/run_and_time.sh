...
ENABLE_CPU_EXCLUSIVE=${ENABLE_CPU_BINDING:-1}
...
if [ -n "${SLURM_LOCALID-}" ]; then
  ...
  if [ "${SLURM_NTASKS}" -gt "${SLURM_JOB_NUM_NODES}" ]; then
    IB_BIND=''
    #SD#if [[ "${SLURM_JOB_NUM_NODES}" -gt 1 && "${ENABLE_IB_BINDING}" -eq 1 ]]; then
    #SD#  IB_BIND='--ib=single'
    #SD#fi
    #SD#if [[ "${ENABLE_CPU_EXCLUSIVE}" -eq 1 ]]; then
    #SD#  CPU_EXCLUSIVE='--cpu=exclusive'
    #SD#fi
    #SD#CMD=( 'bindpcie' ${CPU_EXCLUSIVE} ${IB_BIND} '--' ${PRE_CMD[@]} 'python' '-u' )
