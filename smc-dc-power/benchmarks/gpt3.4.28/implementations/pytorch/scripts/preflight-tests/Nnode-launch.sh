...
#vars with defaults:
: "${CONTAINER:=/lustre/fsw/portfolios/coreai/projects/coreai_mlperf_training/containers/dl+mlperf+optimized+large_language_model.pytorch.10082486.sqsh}"
...
if [[ $CONTAINER =~ "large_language_model.pytorch.10082486" ]]; then
    MLPERF_VERSION=v31
else
    MLPERF_VERSION=v40
fi
...
case $1 in
    ...
    e2e)
        ...
        NNODES=${2}
        GA=${3}
        CONFIG=${4:-config_DGXH100_Nx8xMx4x8_mbs1.sh}
        LOGDIR=${LOGDIRBASE}/${NNODES}n
        mkdir -p ${LOGDIR}

        # launch one run
        MLPERF_VERSION=${MLPERF_VERSION} DGXNNODES=${NNODES} MINIBS=${GA} LOGDIR=${LOGDIR} bash ${OPTIMIZED}/mlperf_utils/launch-mlperf-benchmark.sh --config ${OPTIMIZED}/large_language_model/pytorch/${CONFIG} --runsub ${OPTIMIZED}/large_language_model/pytorch/run.sub --container ${CONTAINER} --partition ${PARTITION} --npar 1 --slurm-extra=--reservation=$RESERVATION --slurm-extra=--output="${LOGDIR}/slurm-%j.out" ${EXCLUDEARG} ${NODEARG} --no-requeue
        ;;
    *)
