    ...
    def on_train_start(self, trainer: "pl.Trainer", pl_module: "pl.LightningModule") -> None:
        if pl_module.cfg.custom.run_warmup_on_synth_data:
            run_training_warmup(trainer, pl_module.cfg.custom.warmup_train_steps,
                                pl_module.cfg.custom.warmup_validation_steps)
...
class MetricsLogger(Logger):
    def __init__(self, trainer, model, custom_callback, target_val_log_ppl,
        ...
        # XXX: the code below comes as the adjustment for the change made here https://github.com/NVIDIA/NeMo/commit/32ebde1a6acc7b90d795ae764517a1c1a66cbad9
        # Now metric keys are not passed directly to PTL but modified inside nemo.
        ...
        self.experiment = None # dummy field. WAR for the bug that was fixed here https://github.com/Lightning-AI/lightning/commit/281d6a27d1cef6d1ebe3a860a1f362a25bbe8ef7
...
class CustomNLPDDPStrategy(NLPDDPStrategy):
    ...
    def save_checkpoint(self, checkpoint: Dict[str, Any], filepath: _PATH,
        ...
        if self.use_dist_ckpt:
            ...
            # TODO: this falls back to legacy torch checkpointing. Remove this
    ...
    def load_dist_checkpoint(self, path: _PATH, lightning_module: pl.LightningModule) -> Dict[str, Any]:
        ...
        ####### XXX: some magic to match legacy checkpoint
        # TODO: replace `ShardedTensor.key` (values in sharded_state_dict)
        # The structure of a checkpoint differes between 1n and 16n and I still don't know why
        # For 1n it is a dict of tensors but for 16n it is dict of dicts of tensors
        ...
        # XXX: following line is a hack. For some reason dist_checkpointing.load fails to load this key which causes errors in apex down the stream
        # We need to investigate why it happens and fix it
    ...
    def load_checkpoint(self, checkpoint_path: _PATH) -> Dict[str, Any]:
        ...
        if 'pytorch-lightning_version' in ckpt:
            # TODO: remove this branch as it was used for resuming from checkpoint saved during training. Now we have enough resources to run whole training
    ...
    def load_optimizer_state_dict(self, checkpoint):
        # XXX: this is a WAR for https://nvbugswb.nvidia.com/NvBugs5/SWBug.aspx?bugid=4425728&cmtNo=
    ...
    def load_model_state_dict(self, checkpoint, strict=True) -> None:
        ...
        if self.mcore_gpt:
            # TODO: this should be the only path once we verify that mcore_gpt works
            # XXX: this is just noop. It is not imlpemented in nemo. Instead the model should be loaded in a callback?!
