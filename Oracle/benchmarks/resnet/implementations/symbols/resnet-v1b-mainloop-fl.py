...
from mlperf_log_utils import resnet_max_pool_log, resnet_conv2d_log, resnet_batchnorm_log, \
...
def residual_unit_norm_conv(rank, data, nchw_inshape, num_filter, stride, dim_match, name, bottle_neck=True,
    ...
    if bottle_neck:
        ...
        mllogger.event(mllogger.constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor=f"{name}_bn1_beta"))
        mllogger.event(mllogger.constants.WEIGHTS_INITIALIZATION, metadata=dict(tensor=f"{name}_bn1_gamma"))
